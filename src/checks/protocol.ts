...
function schemaLooksValid(schema: unknown): { ok: boolean; reason?: string } {
  if (schema == null) {
    return { ok: false, reason: "missing inputSchema" };
  }
  if (!isRecord(schema)) {
    return { ok: false, reason: "inputSchema is not an object" };
  }
  if (schema.type !== "object") {
    return {
      ok: false,
      reason: `expected type "object", got ${String(schema.type)}`,
    };
  }
...
export async function runToolsSchemaCheck(client: Client): Promise<CheckResult> {
  ...
  try {
    ...
    for (const tool of tools) {
      ...
      if (!schemaCheck.ok) {
        issues.push(`${tool.name}: ${schemaCheck.reason}`);
      }
    }
    ...
    return {
      ...
      detail: issues.slice(0, 8).join("; ") + (issues.length > 8 ? ` (+${issues.length - 8} more)` : ""),
      durationMs: Date.now() - started,
...
export async function runLatencyCheck(client: Client): Promise<CheckResult> {
  ...
  try {
    ...
    const severity = max > 3000 ? "warn" : "pass";
    return {
      id: "latency",
...
export async function runPingCheck(client: Client): Promise<CheckResult> {
  ...
  try {
    ...
    return {
      ...
      title: "Ping",
      severity: "warn",
      detail: `Ping failed or unsupported: ${
        error instanceof Error ? error.message : String(error)
      }`,
      durationMs: Date.now() - started,
    };
